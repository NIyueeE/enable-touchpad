...
| `assets/` | 图标资产:多尺寸 `icon.ico`(exe 与托盘资源)、`icon_32.png`(窗口图标)、`icon_16.png`(光标徽标)、`src/` 原始设计图、`make_icons.py` 再生成脚本 |
...
| `etp-ffi/` | 仅 Windows 的 FFI 叶子 crate:精确式触摸板状态查询 + 鼠标层光标徽标覆盖窗(隔离 `unsafe`)|
